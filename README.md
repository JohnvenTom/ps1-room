...
> 一个**单 HTML 文件、零网络请求**的 PlayStation 1 渲染管线还原演示：在浏览器里用 Three.js + 手写 GLSL 完整复刻 PS1 时代的仿射纹理映射（Affine Texture Mapping）、顶点吸附抖动（Vertex Wobble）、RGB555 色深 + Bayer 抖动、低分辨率渲染与全套 CRT 滤镜，并附带实时点光源阴影与第一人称漫游。
...
| **场景** | 8×8×3.2 m 密封房间：木箱堆、油桶、吊锥灯、封板假窗、墙顶管道+弯头、通风格栅 |
| **相机** | 待机微漂移展示（让蠕动始终可见）⇄ 第一人称漫游（无指针锁定依赖），3 秒无操作平滑交还 |
| **工程** | 单文件 ~690 KB、零依赖、零网络请求、全部资源运行时程序化生成 |
...
- 材质设 `lights: true`，uniform 合并 `THREE.UniformsLib.lights`，着色器注入 `<shadowmap_pars_vertex>` / `<shadowmap_vertex>` / `<shadowmap_pars_fragment>` 等官方 chunk（几何为世界坐标烘焙，故 `worldPosition = vec4(position, 1.0)`）；
...
- **接触阴影**：道具脚下贴地 12 mm 的硬轮廓抖动片（木箱=矩形、油桶=八边形，比底面大 8%），走同一顶点吸附管线，静态不随闪烁变化。

投影体：木箱×3、油桶、管道+弯头、灯杆+灯罩、窗板；灯泡（自发光）与贴墙的格栅/玻璃不投影。
...
漫游模式不依赖 Pointer Lock（部分环境会拒绝锁定）：点击或 <kbd>M</kbd> 直接切换行走状态，鼠标转向使用普通 `mousemove` 的 `movementX/Y`（能锁定时会额外锁定）。带墙体钳制、道具 AABB 碰撞（人体半径 0.35 m）、步伐起伏；停止输入 3 秒后以四元数 slerp + 位置 lerp 平滑交还待机漂移。

### 程序化贴图

7 张 64×64 贴图全部在运行时用 JS 像素循环生成（45° 斜格地砖、水泥板墙+污渍流痕、木板+钉头、油桶箍环+铆钉+锈斑、管道焊缝+螺栓、格栅叶片、封板夜窗），另加 1 张 4×4 Bayer 矩阵与 1 张 1×1 纯白（灯泡）。因此"零网络"约束下不存在任何外部素材文件，贴图风格参考自 PS1 时代实机截图。
...
```
ps1-room/
├── index.html            # 最终产物：单文件、零依赖（≈690 KB）
