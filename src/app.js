(function () {
  ...
  var ROOM_VS = [
    ...
    '  vec4 worldPosition = vec4(position, 1.0);',   // geometry is baked in world space
  ...
  // ==================== hard contact-shadow blobs =====================
  // Bayer-dithered flat-black quads hugging the floor under floor props.
  var blobMat = new THREE.ShaderMaterial({
    uniforms: { uSnap: shared.uSnap, uRes: shared.uRes, uBayer: shared.uBayer },
    vertexShader: [
      'uniform float uSnap;',
      'uniform vec2 uRes;',
      'void main() {',
  ...
  function blobRect(cx, cz, sx, sz, rotY) {
    var y = 0.012, hx = sx / 2, hz = sz / 2;
    ...
    blobGroup.add(new THREE.Mesh(g, blobMat));
  }
  function blobOct(cx, cz, r) {
    var y = 0.012, pos = [cx, y, cz], idx = [];
    for (var i = 0; i <= 8; i++) {
  ...
  blobRect(-2.56, -3.40, 1.07, 1.07);        // crate1 + rotated top crate
  blobRect(-1.65, -3.50, 0.92, 0.92);        // crate2
  blobOct(-3.35, -0.60, 0.44);               // drum
